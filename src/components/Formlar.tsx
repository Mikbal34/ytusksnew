...
import { ArrowLeft, Search, Download, ExternalLink } from 'lucide-react';
...
export function Formlar() {
  ...
  useEffect(() => {
    async function fetchFormlar() {
      try {
        setLoading(true);
        // Sadece Kulüp kategorisindeki formları getir
        const data = await formlariGetir('Kulüp');
        setFormlar(data);
        setError(null);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Kulüp Formları</h1>
