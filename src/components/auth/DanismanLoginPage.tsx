...
const DanismanLoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Danışman girişi başlatılıyor...');
      ...
      try {
        ...
        // Danışman değilse hata ver
        if (userProfile.role !== 'danisman') {
          console.error('Kullanıcı rolü danışman değil:', userProfile.role);
          throw new Error('Bu sayfaya erişim için akademik danışman yetkisi gerekiyor');
        }

        console.log('Danışman rolü doğrulandı');
        ...
        // Danışman paneline yönlendir
        console.log('Danışman paneline yönlendiriliyor...');
        navigate('/danisman-paneli');
      } catch (profileError: any) {
        console.error('Profil alınırken hata oluştu:', profileError);
        
        if (profileError.message.includes('danışman yetkisi')) {
      ...
      if (error.message === 'Bu sayfaya erişim için akademik danışman yetkisi gerekiyor') {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-orange-200 p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        <div className="flex flex-col items-center mb-6">
          <Link to="/" className="self-start text-orange-600 hover:text-orange-800 mb-6 flex items-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              placeholder="danisman@yildiz.edu.tr"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2 px-4 border border-transparent rounded-lg shadow-sm text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 flex items-center justify-center"
          >
            {loading ? (
              <>
...
export default DanismanLoginPage; 